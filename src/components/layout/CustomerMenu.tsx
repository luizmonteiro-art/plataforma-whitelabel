...
import {
  ...
  Phone, MessageCircle, Package, Clock, AtSign, ChevronRight, Store,
} from 'lucide-react'
import { cn } from '@/lib/utils'
import type { Brand } from '@/lib/brand'
import { waLink } from '@/lib/brand'

type MenuItem = { icon: React.ElementType; label: string; href: string; msg?: string; external?: boolean }

export function CustomerMenu({ brand }: { brand: Brand }) {
  ...
  const contato: MenuItem[] = []
  if (brand.phone) contato.push({ icon: Phone, label: brand.phone, href: `tel:${brand.phone.replace(/[^\d+]/g, '')}`, external: true })
  contato.push({ icon: MapPin, label: 'Nossa localização', href: '/sobre', msg: 'Olá! Preciso do endereço da loja.' })
  if (brand.hoursWeekday) contato.push({ icon: Clock, label: `Seg–Sex ${brand.hoursWeekday}`, href: '/sobre' })
  if (brand.instagramUrl) contato.push({ icon: AtSign, label: `@${brand.instagram}`, href: brand.instagramUrl, external: true })
  ...
  const whatsappHref = (msg: string) => (hasWa ? waLink(brand, msg) : '#')
          ...
          <div className="absolute left-0 top-full mt-2 w-[280px] sm:w-[300px] rounded-2xl bg-[#111] border border-white/[0.08] shadow-2xl shadow-black/60 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
              <div className="flex items-center gap-2">
                {brand.logoUrl ? (
                  <Image src={brand.logoUrl} alt={brand.name} height={22} width={22} className="object-contain rounded-sm" />
                ) : (
                  <Store size={16} className="text-[var(--accent)]" />
                )}
                <span className="text-sm font-black text-white truncate max-w-[160px]">{brand.name}</span>
              </div>
              <span className="text-[10px] text-zinc-600">Loja</span>
            </div>
                  ...
                  {section.items.map(item => {
                    ...
                    const content = (
                      <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.04] active:bg-white/[0.08] transition-all group/item">
                        <div className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center shrink-0 group-hover/item:border-[var(--accent)]/20 transition-all">
                        ...
                        {item.msg && hasWa && (
                          <button
                            ...
                            className="shrink-0 p-1.5 rounded-lg bg-[var(--accent)]/10 text-[var(--accent)] hover:bg-[var(--accent)]/20 active:scale-90 transition-all"
                            title="Perguntar pelo WhatsApp"
                          >
                            <MessageCircle size={11} />
                          </button>
                        )}
                        {(!item.msg || !hasWa) && <ChevronRight size={12} className="text-zinc-700 group-hover/item:text-zinc-500 shrink-0" />}
                      </div>
                    )
            ...
            {hasWa && (
              <div className="p-3 border-t border-white/[0.06]">
                <a
                  href={whatsappHref(`Olá! Vim pelo site da ${brand.name} e gostaria de mais informações.`)}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-3 bg-[var(--accent)] hover:bg-[var(--accent)] active:scale-95 text-white font-bold rounded-xl transition-all text-sm shadow-lg shadow-[var(--accent)]/20"
                >
...
export function WhatsAppFloat({ brand }: { brand: Brand }) {
  ...
  return (
    ...
    >
      <div className={cn(
        'relative flex items-center gap-2.5 bg-[var(--accent)] hover:bg-[var(--accent)] active:scale-90 rounded-full shadow-xl shadow-[var(--accent)]/30 transition-all duration-300 group-hover:shadow-[var(--accent)]/50',
        ...
        {pulse && <span className="text-white text-xs font-bold whitespace-nowrap">Fale conosco</span>}
      </div>
    </a>
  )
}
