-- ═══════════════════════════════════════════════════════════════════
-- SEED — 20 Produtos M CELL (mmcell)
-- Cole no Supabase Dashboard → SQL Editor → New query → Run
-- ═══════════════════════════════════════════════════════════════════

-- Garante que o insert passe mesmo com RLS ativo
alter table products disable row level security;

-- Remove produtos do seed anterior (se existirem) para evitar duplicatas
delete from products where store_id = 'mmcell';

-- ─── Inserção dos 20 produtos ─────────────────────────────────────
...
(
  ...
  'Carregamento ultra-rápido 45W. Compatible com Galaxy S23, S24, Note e linha A premium. Cabo USB-C incluso. Carga de 0 a 70% em apenas 30 minutos.',
  189.00, NULL, 15,
...
-- ─── Resultado ────────────────────────────────────────────────────
select count(*) as total_produtos, condition, category
from products
where store_id = 'mmcell'
